...
const RepairCompleted = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const assetList: RepairAssetItem[] = Array.isArray(parsed.repairAsset)
        ? parsed.repairAsset
        : [];
      const foundAsset = assetList.find(
        (item: RepairAssetItem) => String(item.id) === String(id)
      );
      if (foundAsset) {
        setAsset(foundAsset);
  ...
  const addNewItem = async () => {
    ...
    allData.repairAsset = allData.repairAsset.filter(
      (item: RepairAssetItem) => String(item.id) !== String(id)
    );
              ...
              >
                ...
                <AccordionItem value="2">
                  <AccordionHeader
                    ...
                    size={"extra-large"}
                  >
                    <h4 className={style2.formSectionTitle}>
                      Thông tin sửa chữa
                    </h4>
                  </AccordionHeader>
                  <AccordionPanel className={style.formContent}>
