...
import AddNewAssetStyle, {
  FormAddMasterDataStyle,
  FormRevoke,
} from "../../../styles/formAdd/formAdd.ts";
...
const MaintenanceCompleted = () => {
  const { id } = useParams();
  // call api lấy data
  const [asset, setAsset] = useState<MaintenanceAssetItem | null>(null);
  ...
  const [formData, setFormData] = useState({
    ...
    taiSan: {
      ...
      hanBaoDuong: asset?.taiSan.hanBaoDuong,
      trangThai: "Hoàn thành bảo dưỡng",
    },
  ...
  const fetchData = async () => {
    try {
      ...
      const assetList: MaintenanceAssetItem[] = Array.isArray(
        parsed.maintenanceAsset
      )
        ? parsed.maintenanceAsset
        : [];
      const foundAsset = assetList.find(
        (item: MaintenanceAssetItem) => String(item.id) === String(id)
      );
      if (foundAsset) {
        setAsset(foundAsset);
        ...
        setFormData({
          ...
          taiSan: {
            ...
            hanBaoDuong: foundAsset.taiSan.hanBaoDuong,
  ...
  const addNewItem = async () => {
    ...
    allData.maintenanceAsset = allData.maintenanceAsset.filter(
      (item: MaintenanceAssetItem) => String(item.id) !== String(id)
    );
    allData.dataTable = allData.dataTable.map((item: TypeTaiSan) =>
      item.id === asset?.taiSan.id
        ? {
            ...item,
            trangThai: "Đang sử dụng",
            lichSuBaoTri: "Bỏa dưỡng hoàn thành vào " + formData.ngayHoanThanh,
          }
        : item
    );
    localStorage.setItem("data", JSON.stringify(allData));
    alert(`Hoàn thành bảo dưỡng`);
    nav(`/asset/maintenance`, { state: { reload: true } });
  };
  const handleDateChange =
    (field: keyof typeof formData) => (date: Date | null | undefined) => {
      setFormData((prev) => ({
        ...prev,
      ...
      <Dialog open={true}>
        <DialogSurface className={style2.formAddSettingWitdh}>
          <DialogBody className={style2.formAddSettingMaster}>
            <DialogTitle>Hoàn thành bảo dưỡng</DialogTitle>
            <DialogContent>
              <Accordion
                openItems={openItems}
                    ...
                    <div>
                      <div className={style.formDiv}>
                        ...
                        <div className={style3.itemInfo}>
                          ...
                          <Text>
                            {asset?.taiSan.nguyenGia.toLocaleString("vi-VN")}
                          </Text>
                        </div>
                        <div className={style3.itemInfo}>
                          <div className={style3.itemInfoIcon}>
                ...
                <AccordionItem value="2">
                  ...
                  <AccordionPanel className={style.formContent}>
                    ...
                    <div>
                      <div className={style.formDiv}>
                        <Field
                          label="Ngày hoàn thành bảo dưỡng"
                          className={style.formLabel}
                        >
                          <DatePicker
                            className={style.formInput}
                            placeholder="Chọn ngày hoàn thành bảo dưỡng tài sản"
                            id={"ngayHoanThanh"}
                            name={"ngayHoanThanh"}
                      ...
                      <div className={style.formDiv}>
                        <Field size="medium" label="Chi phí bảo dưỡng">
                          <Input
                            name={"chiPhiThucTe"}
                            onChange={(e) =>
                              setFormData({
