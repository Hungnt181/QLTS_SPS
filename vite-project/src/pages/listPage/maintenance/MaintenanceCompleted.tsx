...
const MaintenanceCompleted = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const assetList: MaintenanceAssetItem[] = Array.isArray(parsed.maintenanceAsset) ? parsed.maintenanceAsset : [];
            const foundAsset = assetList.find((item: MaintenanceAssetItem) => String(item.id) === String(id));
            if (foundAsset) {
                setAsset(foundAsset);
    ...
    const addNewItem = async () => {
        ...
        allData.maintenanceAsset = allData.maintenanceAsset.filter((item: MaintenanceAssetItem) => String(item.id) !== String(id));
        ...
        nav(`/taisan/maintenance`, {state: {reload: true}})
    }
                            ...
                            >
                                ...
                                <AccordionItem value="2">
                                    ...
                                    <AccordionPanel className={style.formContent}>
                                        ...
                                        <div>
                                            <div className={style.formDiv}>
                                                <Field label="Ngày hoàn thành bảo dưỡng" className={style.formLabel}>
                                                    <DatePicker
                            ...
                            <div className={style.Formbutton}>
                                <DialogTrigger disableButtonEnhancement>
                                    <Button icon={<ChevronLeft16Regular/>} className={style.closeBtn}
                                            appearance="secondary" onClick={() => nav(`/taisan/maintenance`)}>Quay
                                        lại</Button>
                                </DialogTrigger>
